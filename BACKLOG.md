...
Two items need Mauri's own accounts and cannot be done unattended; they are
marked **[needs you]**.

---

## Blocking the quality of everything else

### 1. Add a credit card to Fly — **[needs you]**

The account is on a trial, which stops the machine every five minutes
regardless of traffic. Measured: no downtime (122/122 requests succeeded, the
proxy holds the request and restarts the machine) but roughly one request in
sixty pays 2–3 s, and `/v1/stats` recomputes its cache after every restart.

<https://fly.io/trial>. Nothing else on this list is worth tuning while the
machine cycles.

### 2. Keep the Cloudflare proxy off for `chess.mauriulloa.com`
...
### 4. A static social preview image
...
### 5. Landing pages per theme and level
...
### 6. Shareable puzzle URLs — `/puzzle/{id}`
...
### 9. Submit the API to the usual lists
...
### 10. A piece-count filter — the finding that matters
...
### 11. The printable sheet itself — `/worksheet`
...
### 13. Theme descriptions on `/v1/themes`
...
### 14. Revisit the rate limit with real data
